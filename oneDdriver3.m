% cutfem_1d_marked_driver.m  (self-contained)
% 10 elements on [0,10]. Pp CG (p=1,2,3), symmetric Nitsche at two
% internal Dirichlet points c1 (u=1) and c2=c1+1 (u=0). Source f=10 only
% on (c1,c2]. Strong Dirichlet at x=0 and x=10. Ghost penalty optional.
% Plots: element edges (vertical lines), moving boundary markers,
% per-element colored curves, and markers at element endpoints on the u-curve.

clear; clc; close all;

%% ===== Controls =====
L        = 1.1;         % domain [0, L]
Ne       = 3;           % <-- exactly 10 elements (fixed)
pDeg     = 3;            % polynomial degree: 1,2,or 3
kappa    = 1.0;          % diffusion (uniform)
fVal     = 10.0;         % source magnitude inside the active segment
segLen   = 1.0;          % active segment length (fixed to 1 in your setup)

c0       = 0.05;         % base (left) pin position
shift    = 0.0;          % translate the segment by this distance
gammaN   =6;           % Nitsche penalty (20–100 typical)
useGhost = true;         % ghost penalty near each cut
gammaG   = 0.5;          % ghost coefficient
tiny     = 1e-12;
% =====================

% Derived: translated pins
c1 = c0 + shift;                    % u(c1)=1
c2 = c1 + segLen;                   % u(c2)=0
assert(c1>0 && c2<L, 'Segment must lie strictly inside (0,L).');

% Uniform mesh with exactly 10 elements
xe = linspace(0, L, Ne+1).';        % (Ne+1)x1 node coordinates
...
% [K,F] = apply_dirichlet_rc(K,F, ndof,   0.0);   % u(10)=0

%% --- Solve ---
u = K \ F;

%% --- Plot, with element edges + per-element colors + endpoint markers ---
figure('Color','w'); ax = gca; hold(ax,'on'); grid(ax,'on');
xlabel(ax,'x'); ylabel(ax,'u(x)'); set(ax,'LineWidth',1.25);
ylim(ax,[-0.1, 2.0]); xlim(ax,[0, L]);

% 1) draw vertical lines at ALL element edges
for i = 1:nd_nodes
    xline(ax, xe(i), ':', 'Color', [0.7 0.7 0.7], 'LineWidth', 0.8);
end

% 2) exact reference on the active segment (for visual check, black)
u_exact = @(x) (x<=c1).*1 ...
...
% 3) FEM curve per element, each element a different color; mark endpoints
colors = lines(ne);
for e = 1:ne
    xL = xe(e); xR = xe(e+1);
    ...
    % mark endpoints on the u-curve for this element
...
% 4) mark the moving boundary points with distinct markers
plot(ax, c1, 1.0, 's', 'MarkerSize', 8, 'MarkerFaceColor',[0.2 0.7 0.2], ...
     'MarkerEdgeColor','k', 'DisplayName','c_1 (u=1)');
plot(ax, c2, 0.0, 'd', 'MarkerSize', 8, 'MarkerFaceColor',[0.8 0.2 0.8], ...
     'MarkerEdgeColor','k', 'DisplayName','c_2 (u=0)');

title(ax, sprintf('CutFEM 1D | Ne=10, p=%d, Nitsche=%g, ghost=%d (\\gamma_g=%g)', ...
...
function [K,F] = apply_dirichlet_rc(K,F,i,uD)
    % Strong Dirichlet at node i (row/col zeroing).
    F = F - K(:,i) * uD;
